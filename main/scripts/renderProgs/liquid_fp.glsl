...
void main()
{
	...
	//vec3 N2 = tangentToWorldMatrix * (2.0 * (texture2D(u_NormalMap, texNormal).xyz - 0.5));
	...
	//float fresnel = u_FresnelBias * pow(clamp(dot(N2, N3), 0.0, 1.0), u_FresnelPower) * u_FresnelScale;
	...
	vec3 L = normalize(tangentToWorldMatrix * var_LightDirection);
