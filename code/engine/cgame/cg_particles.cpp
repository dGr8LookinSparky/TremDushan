...
void CG_AddParticleToScene( cparticle_t* p, vec3_t org, vec4_t color )
{
    ...
    else if( p->type == P_FLAT_SCALEUP )
    {
        ...
        sinR = height * sin( DEG2RAD( p->roll ) ) * ( float )sqrt( ( float )2 );
        cosR = width * cos( DEG2RAD( p->roll ) ) * ( float )sqrt( ( float )2 );
            ...
            if( p->bounceFactor )
            {
                ...
                if( trace.fraction > 0 && trace.fraction < 1 )
                {
                    ...
                    if( p->type == P_BLOOD )
                    {
                        ...
                        CG_ImpactMark( shader, trace.endpos, trace.plane.normal, random() * 360, 1, 1, 1, 1, true, radius,
                                       false );
                    }
...
static bool ValidBloodPool( vec3_t start )
{
