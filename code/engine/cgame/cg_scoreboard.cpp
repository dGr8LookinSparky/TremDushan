...
void CG_DrawScoreboardTitlebarNew( vec4_t color, bool team )
{
    const char*     s;
    ...
    if( team )
    {
        ...
        CG_Text_PaintAligned( 320, 93, s, 0.4f, UI_CENTER | UI_DROPSHADOW, colorWhite, &cgs.media.freeSansFont );
...
void CG_DrawScoreboardHeadlineNew( int pos[] )
{
    ...
    CG_Text_PaintAligned( pos[0], 138, s, 0.2f, UI_LEFT | UI_DROPSHADOW, colorWhite, &cgs.media.freeSansFont );
    
    s = va( "Clan" );
    CG_Text_PaintAligned( pos[1], 138, s, 0.2f, UI_LEFT | UI_DROPSHADOW, colorWhite, &cgs.media.freeSansFont );
    
    s = va( "Score" );
    CG_Text_PaintAligned( pos[2], 138, s, 0.2f, UI_LEFT | UI_DROPSHADOW, colorWhite, &cgs.media.freeSansFont );
    
    s = va( "Time" );
    CG_Text_PaintAligned( pos[3], 138, s, 0.2f, UI_LEFT | UI_DROPSHADOW, colorWhite, &cgs.media.freeSansFont );
    
    s = va( "Ping" );
    CG_Text_PaintAligned( pos[4], 138, s, 0.2f, UI_LEFT | UI_DROPSHADOW, colorWhite, &cgs.media.freeSansFont );
...
void CG_DrawScoreboardUnderlineNew( void )
{
    ...
    if( cgs.gametype >= GT_TEAM )	// team based scoreboard
    {
        ...
        CG_Text_PaintAligned( 294, 408, s, 0.2f, UI_RIGHT | UI_DROPSHADOW, colorWhite, &cgs.media.freeSansFont );
        
        // time left
        if( cgs.timelimit > 0 )
        {
        
            CG_Text_PaintAligned( 344, 408, ts, 0.2f, UI_LEFT | UI_DROPSHADOW, colorWhite, &cgs.media.freeSansFont );
        }
        ...
        if( cgs.gametype >= GT_CTF )
        {
            ...
            CG_Text_PaintAligned( 576, 408, s, 0.2f, UI_RIGHT | UI_DROPSHADOW, colorWhite, &cgs.media.freeSansFont );
        }
        else
        {
    ...
    else						// ffa
    {
        ...
        if( cgs.timelimit > 0 )
        {
            ...
            CG_Text_PaintAligned( 320, 408, ts, 0.2f, UI_CENTER | UI_DROPSHADOW, colorWhite, &cgs.media.freeSansFont );
        }
        
        s = va( "FFA on %s", mapname );
        CG_Text_PaintAligned( SCOREBOARD_FFA + 15, 408, s, 0.2f, UI_LEFT | UI_DROPSHADOW, colorWhite, &cgs.media.freeSansFont );
                               ...
                               int pos[] )
{
    ...
    else
    {
        ...
        w = CG_Text_Width( s, fontsize, 0, &cgs.media.freeSansFont );
...
bool CG_DrawScoreboardNew( void )
{
    float           fade;
    float*          fadeColor;
    qtime_t         tm;
    ...
    if( cgs.gametype >= GT_TEAM )	// team based scoreboard
    {
        ...
        CG_DrawScoreboardTitlebarNew( basecolor, true );
    ...
    else						// FFA Scoreboard
    {
        ...
        // Titlebar
        CG_DrawScoreboardTitlebarNew( basecolor, false );
    ...
    trap_RealTime( &tm );
    Com_sprintf( st, sizeof( st ), "%2i:%s%i:%s%i (%i %s %i) " S_COLOR_RED "Legacy v" PRODUCT_VERSION " " S_COLOR_WHITE " http://OpenWolf.sourceforge.net", ( 1 + ( tm.tm_hour + 11 ) % 12 ),	// 12 hour format
    ...
    CG_Text_PaintAligned( 320, 470, st, 0.2f, UI_CENTER | UI_DROPSHADOW, colorWhite, &cgs.media.freeSansFont );
...
bool CG_DrawOldScoreboard( void )
{
    ...
    if( cg.killerName[0] && cg.predictedPlayerState.pm_type != PM_INTERMISSION && cg.predictedPlayerState.pm_type == PM_DEAD )
    {
        ...
        x = ( SCREEN_WIDTH - w ) / 2;
        y = 40 + 70;
        CG_DrawStringExt( x, y, s, colorWhite, false, true, BIGCHAR_WIDTH, BIGCHAR_HEIGHT, 900 );
    }
    else
    {
        s = va( "%s place with %i", CG_PlaceString( cg.snap->ps.persistant[PERS_RANK] + 1 ), cg.snap->ps.persistant[PERS_SCORE] );
    ...
    w = CG_Text_Width( st, 0.3f, 0, &cgs.media.freeSansFont );
    x = 320 - w / 2;
    y = 475;
    CG_Text_Paint( x, y, 0.3f, colorWhite, st, 0, 0, UI_DROPSHADOW, &cgs.media.freeSansFont );
