...
==========================
*/
bool CG_FindClientModelFile( char* filename, int length, clientInfo_t* ci, const char* modelName,
...
==========================
*/
bool CG_RegisterClientModel( clientInfo_t* ci, const char* modelName, const char* skinName, const char* teamName )
{
    int             i;
    ...
    if( ci->bodyModel )
    {
        ...
        CG_RegisterPlayerAnimation( ci, modelName, BOTH_DEATH1, "die", false, false, false );
        //CG_RegisterPlayerAnimation(ci, modelName, BOTH_DEATH2, "death2", false, false, false);
        //CG_RegisterPlayerAnimation(ci, modelName, BOTH_DEATH3, "death3", false, false, false);
        ...
        if( !CG_RegisterPlayerAnimation( ci, modelName, TORSO_ATTACK2, "idle", false, false, false ) )
            ci->animations[TORSO_ATTACK2] = ci->animations[LEGS_IDLE];
            
        if( !CG_RegisterPlayerAnimation( ci, modelName, TORSO_STAND2, "idle", false, false, false ) )
            ci->animations[TORSO_STAND2] = ci->animations[LEGS_IDLE];
            
        if( !CG_RegisterPlayerAnimation( ci, modelName, LEGS_IDLECR, "crouch", true, false, false ) )
            ci->animations[LEGS_IDLECR] = ci->animations[LEGS_IDLE];
            
        if( !CG_RegisterPlayerAnimation( ci, modelName, LEGS_WALKCR, "crouch_forward", true, false, true ) )
            ci->animations[LEGS_WALKCR] = ci->animations[LEGS_IDLE];
            
        if( !CG_RegisterPlayerAnimation( ci, modelName, LEGS_BACKCR, "crouch_forward", true, true, true ) )
            ci->animations[LEGS_BACKCR] = ci->animations[LEGS_IDLE];
            
        if( !CG_RegisterPlayerAnimation( ci, modelName, LEGS_WALK, "walk", true, false, true ) )
            ci->animations[LEGS_WALK] = ci->animations[LEGS_IDLE];
            
        if( !CG_RegisterPlayerAnimation( ci, modelName, LEGS_BACKWALK, "walk_backwards", true, false, true ) )
            ci->animations[LEGS_BACKWALK] = ci->animations[LEGS_IDLE];
            
        if( !CG_RegisterPlayerAnimation( ci, modelName, LEGS_RUN, "run", true, false, true ) )
            ci->animations[LEGS_RUN] = ci->animations[LEGS_IDLE];
            
        if( !CG_RegisterPlayerAnimation( ci, modelName, LEGS_BACK, "run_backwards", true, true, true ) )
            ci->animations[LEGS_BACK] = ci->animations[LEGS_IDLE];
            
        if( !CG_RegisterPlayerAnimation( ci, modelName, LEGS_SWIM, "swim", true, false, true ) )
        ...
        if( !CG_RegisterPlayerAnimation( ci, modelName, LEGS_JUMPB, "jump", false, true, false ) )
            ci->animations[LEGS_JUMPB] = ci->animations[LEGS_IDLE];
            
        if( !CG_RegisterPlayerAnimation( ci, modelName, LEGS_LANDB, "land", false, true, false ) )
            ci->animations[LEGS_LANDB] = ci->animations[LEGS_IDLE];
            
        if( !CG_RegisterPlayerAnimation( ci, modelName, LEGS_TURN, "step", false, false, false ) )
...
==================
*/
void CG_SwingAngles( float destination, float swingTolerance, float clampTolerance, float speed, float* angle, bool* swinging )
{
    float           swing;
    float           move;
    float           scale;
...
bool CG_PlayerShadow( centity_t* cent, float* shadowPlane, int noShadowID )
{
...
void CG_Player( centity_t* cent )
{
    ...
    if( cent->currentState.eFlags & EF_WALLCLIMB && BG_RotateAxis( cent->currentState.angles2, tempAxis, tempAxis2, true, cent->currentState.eFlags & EF_WALLCLIMBCEILING ) )
    ...
    if( cent->currentState.eFlags & EF_WALLCLIMB && BG_RotateAxis( cent->currentState.angles2, body.axis, tempAxis, false, cent->currentState.eFlags & EF_WALLCLIMBCEILING ) )
