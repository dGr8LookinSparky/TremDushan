...
void CG_TestAnimation_f( void )
{
    ...
    if( !trap_R_BuildSkeleton( &cg.testModelEntity.skeleton,
                               cg.testAnimation,
                               cg.testModelEntity.oldframe, cg.testModelEntity.frame, 1.0 - cg.testModelEntity.backlerp, false ) )
    {
...
void CG_TestModelNextFrame_f( void )
{
    ...
    if( cg.testAnimation )
    {
        if( !trap_R_BuildSkeleton( &cg.testModelEntity.skeleton,
                                   ...
                                   cg.testModelEntity.frame, 1.0 - cg.testModelEntity.backlerp, false ) )
        {
            CG_Printf( "Can't build animation\n" );
        }
    }
...
static void CG_WaterBlendBlob( bool inwater )
{
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, bool demoPlayback )
{
    ...
    // draw debugging information for navigation mesh
    CG_NavDraw();
    
    // draw debugging and testing information for detour pathfinding
    CG_PathDraw();
    
    // for debugging and testing automatic calculated waypoints based on navmesh and pathdata
    CG_WaypointDraw();
    
    // world cover representation
    CG_CoverDraw();
