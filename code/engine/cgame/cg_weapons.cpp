...
static bool CG_RegisterWeaponAnimation( animation_t* anim, const char* filename, bool loop, bool reversed,
                                        bool clearOrigin )
{
    int             frameRate;
...
void CG_RegisterWeapon( int weaponNum )
{
    ...
    if( weaponInfo->viewModel )
    {
        ...
        if( !CG_RegisterWeaponAnimation( &weaponInfo->viewModel_animations[WEAPON_READY], path, true, false, false ) )
        {
        ...
        CG_RegisterWeaponAnimation( &weaponInfo->viewModel_animations[WEAPON_RAISING], path, false, false, false );
        
        strcpy( path, item->models[0] );
        Com_StripExtension( path, path, sizeof( path ) );
        strcat( path, "_view_lower.md5anim" );
        CG_RegisterWeaponAnimation( &weaponInfo->viewModel_animations[WEAPON_DROPPING], path, false, false, false );
        
        strcpy( path, item->models[0] );
        Com_StripExtension( path, path, sizeof( path ) );
        strcat( path, "_view_fire.md5anim" );
        CG_RegisterWeaponAnimation( &weaponInfo->viewModel_animations[WEAPON_FIRING], path, true, false, false );
    }
    ...
    switch( weaponNum )
    {
        ...
        case WP_FLAK_CANNON:
            ...
            //weaponInfo->projectileSound = trap_S_RegisterSound( "sound/weapons/flakcannon/wnalflit.ogg", false );
    ...
    if( cent->pe.barrelSpinning == !( cent->currentState.eFlags & EF_FIRING ) )
    {
        ...
        if( cent->currentState.weapon == WP_CHAINGUN && !cent->pe.barrelSpinning )
        {
            trap_S_StartSound( NULL, cent->currentState.number, CHAN_WEAPON,
                               trap_S_RegisterSound( "sound/weapons/vulcan/wvulwind.wav", false ) );
        }
#endif
    }
