        ...
        GLenum shaderType ) const
{
    ...
    if( shaderType == GL_VERTEX_SHADER_ARB )
    {
        Com_sprintf( filename, sizeof( filename ), "scripts/renderProgs/%s_vp.glsl", mainShaderName );
        ri.Printf( PRINT_ALL, "...loading vertex main() shader '%s'\n", filename );
    }
    else
    {
        Com_sprintf( filename, sizeof( filename ), "scripts/renderProgs/%s_fp.glsl", mainShaderName );
        ...
        const std::string& compileMacros ) const
{
    ...
    if( !compileMacros.empty() )
    {
        ...
        while( 1 )
        {
            token = Com_ParseExt( compileMacrosP, false );
            if( !token[0] )
            {
                break;
            }
