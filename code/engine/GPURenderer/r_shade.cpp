...
#include "../../libs/glsl-optimizer/src/scripts/glsl/glsl_optimizer.h"
...
static void GLSL_LoadGPUShader( GLhandleARB program, const char* name, const char* _libs, const char* _compileMacros, GLenum shaderType, bool optimize )
{
    ...
    while( 1 )
    {
        ...
        {
            Com_sprintf( filename, sizeof( filename ), "scripts/renderProgs/%s_vp.glsl", token );
            ri.Printf( PRINT_DEVELOPER, "...loading vertex shader '%s'\n", filename );
        }
        else
        {
            Com_sprintf( filename, sizeof( filename ), "scripts/renderProgs/%s_fp.glsl", token );
        }
    ...
    if( shaderType == GL_VERTEX_SHADER_ARB )
    {
        Com_sprintf( filename, sizeof( filename ), "scripts/renderProgs/%s_vp.glsl", name );
        ri.Printf( PRINT_DEVELOPER, "...loading vertex main() shader '%s'\n", filename );
    }
    else
    {
        Com_sprintf( filename, sizeof( filename ), "scripts/renderProgs/%s_fp.glsl", name );
    ...
    
    {
        ...
        while( 1 )
        {
            token = Com_ParseExt( compileMacros, false );
            if( !token[0] )
            {
                break;
            }
...
static void GLSL_InitGPUShader( shaderProgram_t* program, const char* name, int attribs, bool fragmentShader, bool optimize )
{
...
void GLSL_InitGPUShaders( void )
{
    ...
    GLSL_InitGPUShader( &tr.depthToColorShader, "depthToColor", ATTR_POSITION, true, true );
    ...
    // volumetric lighting
    GLSL_InitGPUShader( &tr.lightVolumeShader_omni, "lightVolume_omni", ATTR_POSITION, true, true );
    ...
    GLSL_InitGPUShader( &tr.volumetricFogShader, "volumetricFog", ATTR_POSITION, true, true );
    ...
    GLSL_InitGPUShader( &tr.screenSpaceAmbientOcclusionShader, "screenSpaceAmbientOcclusion", ATTR_POSITION, true, true );
    ...
    GLSL_InitGPUShader( &tr.depthOfFieldShader, "depthOfField", ATTR_POSITION, true, true );
