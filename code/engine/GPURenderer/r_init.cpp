...
the format is OpenWolf-YYYY_MM_DD-HH_MM_SS-MS.tga/jpeg/png
...
static void RB_TakeScreenshotPNG( int x, int y, int width, int height, char* fileName )
{
    ...
    ri.FS_WriteFile( fileName, buffer, 1 );	// create path
    SavePNG( fileName, buffer, glConfig.vidWidth, glConfig.vidHeight, 3, false );
...
static void R_GenerateMaterialFile_f(void)
{
	...
	FS_Printf(f, "// generated by OpenWolf\n\n");
...
//#endif
    ...
    r_mode = ri.Cvar_Get( "r_mode", "7", CVAR_ARCHIVE | CVAR_LATCH );
    ...
    //AssertCvarRange(r_screenSpaceAmbientOcclusion, 0, 2, true);
#endif
#ifdef EXPERIMENTAL
    ...
    {
        AssertCvarRange( r_hdrRendering, 0, 0, true );
    }
