...
===============
*/
static void R_HDRTonemapLightingColors( const vec4_t in, vec4_t out, bool applyGamma )
{
...
void LoadRGBEToFloats( const char* name, float** pic, int* width, int* height, bool doGamma, bool toneMap,
                       bool compensate )
{
    int             i, j;
    byte*           buf_p;
    ...
    *pic = NULL;
    ...
    while( true )
    {
        token = Com_ParseExt( ( char** )&buf_p, true );
        if( !token[0] )
            break;
        ...
        if( !Q_stricmp( token, "FORMAT" ) )
        {
            ...
            token = Com_ParseExt( ( char** )&buf_p, false );
            if( !Q_stricmp( token, "=" ) )
            {
                token = Com_ParseExt( ( char** )&buf_p, false );
...
#if 0
    w = h = 0;
    LoadRGBEToFloats( name, &hdrImage, &w, &h, true, true, true );
...
#else
    w = h = 0;
    LoadRGBEToFloats( name, &hdrImage, &w, &h, false, false, false );
        ...
        if( tr.worldHDR_RGBE )
        {
                    ...
                    glConfig2.framebufferBlitAvailable && glConfig2.textureFloatAvailable && glConfig2.textureHalfFloatAvailable )
            {
                ...
                for( i = 0; i < numLightmaps; i++ )
                {
                    ...
                    width = height = 0;
                    //LoadRGBEToFloats(va("%s/%s", mapName, lightmapFiles[i]), &hdrImage, &width, &height, true, false, true);
                    LoadRGBEToHalfs( va( "%s/%s", mapName, lightmapFiles[i] ), &hdrImage, &width, &height );
                    
                    //ri.Printf(PRINT_ALL, "...converted '%s/%s' to HALF format\n", mapName, lightmapFiles[i]);
                    
                    image = R_AllocImage( va( "%s/%s", mapName, lightmapFiles[i] ), true );
                    if( !image )
                        break;
...
//  ri.Printf(PRINT_ALL, "ShaderForShaderNum: '%s'\n", dsh->shader);
    ...
    shader = R_FindShader( dsh->shader, SHADER_3D_STATIC, true );
...
static void ParseFace( dsurface_t* ds, drawVert_t* verts, bspSurface_t* surf, int* indexes )
{
    ...
    */
    ...
    for( i = 0; i < numVerts; i++ )
    {
        ...
        R_HDRTonemapLightingColors( cv->verts[i].lightColor, cv->verts[i].lightColor, true );
#endif
    }
...
static void ParseMesh( dsurface_t* ds, drawVert_t* verts, bspSurface_t* surf )
{
    ...
    for( i = 0; i < numPoints; i++ )
    {
        ...
        R_HDRTonemapLightingColors( points[i].lightColor, points[i].lightColor, true );
#endif
    }
...
static void ParseTriSurf( dsurface_t* ds, drawVert_t* verts, bspSurface_t* surf, int* indexes )
{
    ...
    for( i = 0; i < numVerts; i++ )
    {
        for( j = 0; j < numVerts; j++ )
        {
            ...
            if( R_CompareVert( &cv->verts[i], &cv->verts[j], false ) )
            {
...
/*static bool CompareWorldVert(const srfVert_t * v1, const srfVert_t * v2)
{
	int             i;
...
static bool CompareLightVert(const srfVert_t * v1, const srfVert_t * v2)
{
	int             i;
...
static bool CompareShadowVertAlphaTest(const srfVert_t * v1, const srfVert_t * v2)
{
	int             i;
...
static bool CompareShadowVert(const srfVert_t * v1, const srfVert_t * v2)
{
	int             i;
...
static bool CompareShadowVolumeVert( const srfVert_t* v1, const srfVert_t* v2 )
{
    int             i;
...
/*static bool CompareWorldVertSmoothNormal(const srfVert_t * v1, const srfVert_t * v2)
{
	int             i;
...
static int OptimizeVertices( int numVerts, srfVert_t* verts, int numTriangles, srfTriangle_t* triangles, srfVert_t* outVerts,
                             bool( *CompareVert )( const srfVert_t* v1, const srfVert_t* v2 ) )
{
    srfTriangle_t*  tri;
    int             i, j, k, l;
							  ...
							  bool(*compareVert) (const srfVert_t * v1, const srfVert_t * v2))
{
	...
	int             i, j, k, l;
	static int      redundantIndex[MAX_MAP_DRAW_VERTS];
	static          bool(*compareFunction) (const srfVert_t * v1, const srfVert_t * v2) = NULL;
...
//  shaderInfo_t   *si;
    ...
    for( i = 0; i < numTotalVerts; i++ )
    {
        ...
        for( j = i; j < numTotalVerts && numVerts < MAX_SAMPLES; j++ )
        {
            ...
            // test vertexes
            if( CompareWorldVertSmoothNormal( &yDrawVerts[i], &yDrawVerts[j] ) == false )
                continue;
    ...
    if( node->contents != CONTENTS_NODE )
    {
        ...
        */
        ...
        if( node->numMarkSurfaces > 0 )
        {
            ...
            bspSurface_t**  mark;
            srfGeneric_t*   gen;
            bool		mergedSurfBounds;
            
            // add node surfaces to bounds
            mark = node->markSurfaces;
            c = node->numMarkSurfaces;
            ClearBounds( node->surfMins, node->surfMaxs );
            mergedSurfBounds = false;
            while( c-- )
            {
...
//	vec3_t			offset = {0.01, 0.01, 0.01};
    ...
    for( j = 0, out = &s_worldData.nodes[0]; j < s_worldData.numnodes; j++, out++ )
    {
        ...
        Com_Memset( out->visible, false, sizeof( out->visible ) );
...
static void R_LoadFogs( lump_t* l, lump_t* brushesLump, lump_t* sidesLump )
{
    ...
    for( i = 0; i < count; i++, fogs++ )
    {
        ...
        shader = R_FindShader( fogs->shader, SHADER_3D_DYNAMIC, true );
...
void R_LoadLightGrid( lump_t* l )
{
    ...
    for( i = 0; i < w->numLightGridPoints; i++, in++, gridPoint++ )
    {
        ...
        R_HDRTonemapLightingColors( gridPoint->ambientColor, gridPoint->ambientColor, true );
        R_HDRTonemapLightingColors( gridPoint->directedColor, gridPoint->directedColor, true );
#endif
    }
    ...
    while( 1 )
    {
        ...
        while( 1 )
        {
            ...
            else if( !Q_stricmp( keyname, "origin" ) || !Q_stricmp( keyname, "light_origin" ) )
            {
                ...
                s = &value[0];
                Com_Parse1DMatrix( &s, 3, light->l.origin, false );
            }
            // check for center
            else if( !Q_stricmp( keyname, "light_center" ) )
            {
                ...
                s = &value[0];
                Com_Parse1DMatrix( &s, 3, light->l.center, false );
            }
            // check for color
            else if( !Q_stricmp( keyname, "_color" ) )
            {
                ...
                s = &value[0];
                Com_Parse1DMatrix( &s, 3, light->l.color, false );
            }
            // check for radius
            else if( !Q_stricmp( keyname, "light_radius" ) )
            {
                ...
                s = &value[0];
                Com_Parse1DMatrix( &s, 3, light->l.radius, false );
            }
            ...
            else if( !Q_stricmp( keyname, "light_target" ) )
            {
                ...
                s = &value[0];
                Com_Parse1DMatrix( &s, 3, light->l.projTarget, false );
            ...
            else if( !Q_stricmp( keyname, "light_right" ) )
            {
                ...
                s = &value[0];
                Com_Parse1DMatrix( &s, 3, light->l.projRight, false );
                light->l.rlType = RL_PROJ;
            }
            // check for light_up
            else if( !Q_stricmp( keyname, "light_up" ) )
            {
                ...
                s = &value[0];
                Com_Parse1DMatrix( &s, 3, light->l.projUp, false );
                light->l.rlType = RL_PROJ;
            }
            ...
            else if( !Q_stricmp( keyname, "light_start" ) )
            {
                ...
                s = &value[0];
                Com_Parse1DMatrix( &s, 3, light->l.projStart, false );
                light->l.rlType = RL_PROJ;
            }
            // check for light_end
            else if( !Q_stricmp( keyname, "light_end" ) )
            {
                ...
                s = &value[0];
                Com_Parse1DMatrix( &s, 3, light->l.projEnd, false );
                light->l.rlType = RL_PROJ;
            }
            // check for radius
...
//  vec3_t          lightDir;
	...
	for(i = 0, tri = triangles; i < numTriangles; i++, tri++)
	{
		...
		if(tri->neighbors[0] < 0 || !sh.facing[tri->neighbors[0]])
		{
			...
			if(PlaneFromPoints(plane, pos[0], pos[1], pos[2], true))
			{
	...
	for(i = 0, tri = triangles; i < numTriangles; i++, tri++)
	{
		...
		if(PlaneFromPoints(plane, pos[0], pos[1], pos[2], false))
		{
			shadowPlanes[numShadowPlanes].normal[0] = plane[0];
			shadowPlanes[numShadowPlanes].normal[1] = plane[1];
			shadowPlanes[numShadowPlanes].normal[2] = plane[2];
...
static bool R_PrecacheFaceInteraction( srfSurfaceFace_t* cv, shader_t* shader, trRefLight_t* light )
{
                                 ...
                                 trRefLight_t* light )
{
    ...
    for( i = 0, tri = triangles; i < numTriangles; i++, tri++ )
    {
        ...
        if( PlaneFromPoints( tri->plane, pos[0], pos[1], pos[2], true ) )
        {
            tri->degenerated = false;
        ...
        for( i = 0; i < 6; i++ )
        {
            ...
            if( false )
            {
                ...
                if( fileBufY >= REF_CUBEMAP_STORE_SIDE )
                {
                    ...
                    SavePNG( fileName, fileBuf, REF_CUBEMAP_STORE_SIZE, REF_CUBEMAP_STORE_SIZE, 4, false );
...
//	ri.Cmd_ExecuteText(EXEC_NOW, "updatescreen\n");
    ...
    if( tr.sunShaderName )
    {
        tr.sunShader = R_FindShader( tr.sunShaderName, SHADER_3D_STATIC, true );
    }
    //----(SA)  end
