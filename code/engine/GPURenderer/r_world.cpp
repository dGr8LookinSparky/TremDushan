...
static bool R_LightSurfaceGeneric( srfGeneric_t* face, trRefLight_t*   light, byte* cubeSideBits )
{
...
static void R_MarkLeaves( void )
{
    ...
    {
    	r_dynamicBspOcclusionCulling->modified = false;
    }
    */
...
static bool InsideViewFrustum( bspNode_t* node, int planeBits )
{
    if( !r_nocull->integer )
    {
        int             i;
...
static void IssueOcclusionQuery( link_t* queue, bspNode_t* node, bool resetMultiQueryLink )
{
...
static void PullUpVisibility( bspNode_t* node )
{
    ...
    {
        parent->visible[tr.viewCount] = true;
            ...
            if( ResultAvailable( ( bspNode_t* ) QueueFront( &occlusionQueryQueue )->data ) )
            {
                ...
                if( node->occlusionQuerySamples[tr.viewCount] > r_chcVisibilityThreshold->integer )
                {
                    ...
                    if( !QueueEmpty( &node->multiQuery ) )
                    {
                        ...
                        IssueOcclusionQuery( &occlusionQueryQueue, multiQueryNode, false );
                        ...
                        while( !QueueEmpty( &multiQueryNode->multiQuery ) )
                        {
                            ...
                            {
                                IssueOcclusionQuery( &occlusionQueryQueue, node, true );
                            }
                        }
                    }
                    else
                ...
                {
                    node->visible[tr.viewCount] = false;
            ...
            */
              ...
              )
            {
                ...
                if( r_dynamicBspOcclusionCulling->integer > 1 )
                {
                    ...
                    node->visible[tr.viewCount] = ( bool ) !QueryReasonable( node );
                }
                ...
                if( clipsNearPlane )
                {
                    ...
                    node->lastQueried[tr.viewCount] = tr.frameCount;
                    node->visible[tr.viewCount] = true;
