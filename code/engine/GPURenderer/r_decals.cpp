...
static bool MakeTextureMatrix( vec4_t texMat[2], vec4_t projection, decalVert_t* a, decalVert_t* b, decalVert_t* c )
{
    ...
    VectorMA( a->xyz, -d, projection, ( vec_t* )pa );
    d = DotProduct( b->xyz, projection ) - projection[3];
    VectorMA( b->xyz, -d, projection, ( vec_t* )pb );
    d = DotProduct( c->xyz, projection ) - projection[3];
    VectorMA( c->xyz, -d, projection, ( vec_t* )pc );
    ...
    VectorSubtract( ( const vec_t* )xyz, ( const vec_t* )origin, vecs[0] );
    ...
    VectorSubtract( ( const vec_t* )xyz, ( const vec_t* )origin, vecs[1] );
                      ...
                      int fadeTime )
{
    ...
    if( !PlaneFromPoints( temp.planes[0], dv[0].xyz, dv[1].xyz, dv[2].xyz, true ) )
    {
        return;
    }
    ...
    {
        VectorMA( dv[i].xyz, projection[3], projection, xyz );
        if( !PlaneFromPoints( temp.planes[i + 2], dv[( i + 1 ) % numPoints].xyz, dv[i].xyz, xyz, true ) )
        {
            return;
        }
    }
                                     ...
                                     bspModel_t* bmodel )
{
    ...
    if( !PlaneFromPoints( plane, points[0][0], points[0][1], points[0][2], true ) )
    {
        return;
    }
