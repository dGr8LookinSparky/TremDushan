...
static ID_INLINE bool StackEmpty( link_t* l )
{
...
static ID_INLINE bool QueueEmpty( link_t* l )
{
...
typedef struct trRefLight_s
{
    ...
    float           depthNear;
    float           depthFar;
    bool        noDepthBoundsTest;
    
    bool		clipsNearPlane;
    
    bool        noOcclusionQueries;
...
typedef struct
{
    ...
    bool        privatePolygonOffset;	// set for decals and other items that must be offset
...
typedef struct shader_s
{
    ...
    bool		fogVolume;		// surface encapsulates a fog volume
    fogParms_t		fogParms;
...
static ID_INLINE void GLSL_SetUniform_LightParallel( shaderProgram_t* program, bool value )
{
#if defined(USE_UNIFORM_FIREWALL)
...
static ID_INLINE void GLSL_SetUniform_ShadowCompare( shaderProgram_t* program, bool value )
{
#if defined(USE_UNIFORM_FIREWALL)
...
static ID_INLINE void GLSL_SetUniform_ParallaxMapping( shaderProgram_t* program, bool value )
{
#if defined(USE_UNIFORM_FIREWALL)
...
static ID_INLINE void GLSL_SetUniform_PortalClipping( shaderProgram_t* program, bool value )
{
#if defined(USE_UNIFORM_FIREWALL)
...
static ID_INLINE void GLSL_SetUniform_VertexSkinning( shaderProgram_t* program, bool value )
{
#if defined(USE_UNIFORM_FIREWALL)
...
typedef struct bspNode_s
{
    ...
    bool		issueOcclusionQuery[MAX_VIEWS];
...
void            R_SubImageCpy( byte* dest, size_t destx, size_t desty, size_t destw, size_t desth, byte* src, size_t srcw, size_t srch, size_t bytes, bool in );
...
qhandle_t       RE_RegisterShaderFromImage( const char* name, image_t* image, bool mipRawImage );
bool		RE_LoadDynamicShader( const char* shadername, const char* shadertext );

shader_t*       R_FindShader( const char* name, shaderType_t type, bool mipRawImage );
...
void            RE_AddCoronaToScene( const vec3_t org, float r, float g, float b, float scale, int id, bool visible );
