...
static bool ParseHeightMap( char** text, byte** pic, int* width, int* height, int* bits, const char* materialName )
{
    char*           token;
    float           scale;
...
static bool ParseDisplaceMap( char** text, byte** pic, int* width, int* height, int* bits, const char* materialName )
{
    char*           token;
    byte*           pic2;
    int             width2, height2;
...
static bool ParseAddNormals( char** text, byte** pic, int* width, int* height, int* bits, const char* materialName )
{
    char*           token;
    byte*           pic2;
    int             width2, height2;
...
static bool ParseInvertAlpha( char** text, byte** pic, int* width, int* height, int* bits, const char* materialName )
{
    char*           token;
...
static bool ParseInvertColor( char** text, byte** pic, int* width, int* height, int* bits, const char* materialName )
{
    char*           token;
...
static bool ParseMakeIntensity( char** text, byte** pic, int* width, int* height, int* bits, const char* materialName )
{
    char*           token;
...
static bool ParseMakeAlpha( char** text, byte** pic, int* width, int* height, int* bits, const char* materialName )
{
    char*           token;
...
static void R_LoadImage( char** buffer, byte** pic, int* width, int* height, int* bits, const char* materialName )
{
    ...
    *height = 0;
    ...
    token = Com_ParseExt( buffer, false );
    if( !token[0] )
    {
...
//  ri.Printf(PRINT_ALL, "R_FindImageFile: buffer '%s'\n", buffer);
    ...
    else if( r_tryCachedDDSImages->integer && !( bits & IF_NOCOMPRESSION ) && Q_strncasecmp( name, "fonts", 5 ) )
    {
        ...
        // try to load a cached .dds texture from the OpenWolf/<mod>/dds/ folder
        image = R_LoadDDSImage( ddsName, bits, filterType, wrapType );
        if( image != NULL )
        {
    ...
    {
        // try to cache a .dds texture to the OpenWolf/<mod>/dds/ folder
        SavePNG( ddsName, pic, width, height, 4, true );
    }
#endif
...
[____]
[____]
R_SubImageCpy(dest, 0, 0, 4, 4, src, 2, 2, channels, true);
[____]___]
[____]___]
[________]
[________]
