...
//  ri.Printf(PRINT_ALL, "LoadMap: buffer '%s'\n", buffer);
    ...
    token = Com_ParseExt( &buffer_p, false );
...
static bool ParseStage( shaderStage_t* stage, char** text )
{
    ...
    while( 1 )
    {
        ...
        else if( !Q_stricmp( token, "texGen" ) || !Q_stricmp( token, "tcGen" ) )
        {
            ...
            else if( !Q_stricmp( token, "texture" ) || !Q_stricmp( token, "main" ) )
            {
#if !defined(COMPAT_Q3A) && !defined(COMPAT_ET)
...
static char*    CreateShaderByGuide( const char* guideName, char* shaderText )
{
    ...
    if( guideText )
    {
        shader.createdByGuide = true;
        
        // parse guide parameters
        numGuideParms = 0;
        
        token = Com_ParseExt( &guideText, true );
        if( Q_stricmp( token, "(" ) )
        {
            ri.Printf( PRINT_WARNING, "WARNING: expected ( found '%s'\n", token );
            return NULL;
        ...
        token = Com_ParseExt( &shaderText, true );
        if( Q_stricmp( token, "(" ) )
        {
            ri.Printf( PRINT_ALL, "WARNING: expected ( found '%s'\n", token );
            return NULL;
        ...
        while( 1 )
        {
            ...
            while( 1 )
            {
                token = Com_ParseExt( &guideText, false );
...
static bool ParseShader( char* _text )
{
    ...
    while( 1 )
    {
        ...
        else if( !Q_stricmp( token, "ambientLight" ) )
        {
            ...
            shader.ambientLight = true;
            continue;
        }
        // volumetricLight
        else if( !Q_stricmp( token, "volumetricLight" ) )
        {
            shader.volumetricLight = true;
            continue;
        }
        // translucent
        ...
        else if( !Q_stricmp( token, "sunshader" ) )
        {
            ...
            /*
            RB: don't call tr.sunShader = R_FindShader(token, SHADER_3D_STATIC, true);
...
static shader_t* FinishShader( void )
{
    ...
    if( shader.type == SHADER_LIGHT )
    {
        if( stages[0].type != ST_ATTENUATIONMAP_Z )
        {
            ...
            LoadMap( &stages[0], "textures/lights/squarelight1a.tga" );
        }
...
qhandle_t RE_RegisterShader( const char* name )
{
    ...
    sh = R_FindShader( name, SHADER_2D, true );
...
qhandle_t RE_RegisterShaderNoMip( const char* name )
{
    ...
    sh = R_FindShader( name, SHADER_2D, false );
...
qhandle_t RE_RegisterShaderLightAttenuation( const char* name )
{
    ...
    sh = R_FindShader( name, SHADER_LIGHT, false );
...
static void CreateExternalShaders( void )
{
    ...
    tr.flareShader = R_FindShader( "flareShader", SHADER_3D_DYNAMIC, true );
    tr.sunShader = R_FindShader( "sun", SHADER_3D_DYNAMIC, true );
    
    tr.defaultPointLightShader = R_FindShader( "lights/defaultPointLight", SHADER_LIGHT, true );
    tr.defaultProjectedLightShader = R_FindShader( "lights/defaultProjectedLight", SHADER_LIGHT, true );
    tr.defaultDynamicLightShader = R_FindShader( "lights/defaultDynamicLight", SHADER_LIGHT, true );
}
