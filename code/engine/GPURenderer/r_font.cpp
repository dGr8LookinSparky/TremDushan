...
void RE_RegisterFont( const char* fontName, int pointSize, fontInfo_t* font )
{
    ...
    Com_sprintf( fileName, sizeof( fileName ), "gfx/fonts/%s_%i.dat", fontName, pointSize );
#else
...
#if defined(COMPAT_ET) // DON'T DO THIS WITH VANILLA OpenWolf
...
//      Com_Memcpy(font, faceData, sizeof(fontInfo_t));
    ...
    while( i <= GLYPH_END )
    {
        ...
        if( xOut == -1 || yOut == -1 || i == GLYPH_END )
        {
            ...
            {
                SavePNG( fileName, imageBuff, FONT_SIZE, FONT_SIZE, 4, true );
            }
    
            image = R_CreateImage( fileName, imageBuff, FONT_SIZE, FONT_SIZE, IF_NOPICMIP, FT_LINEAR, WT_CLAMP );
            h = RE_RegisterShaderFromImage( fileName, image, false );
