...
extern void     SV_AIFrame( int time ); // AI will run alongside the default bots
...
void CL_Disconnect( bool showMainMenu )
{
    ...
    *clc.downloadTempName = *clc.downloadName = 0;
    ...
    cl_connectedToVoipServer = false;
#endif
...
void CL_ConnectionlessPacket( netadr_t from, msg_t* msg )
{
    ...
    // global MOTD from OpenWolf master server
    if( !Q_stricmp( c, "motd" ) )
    {
        CL_MotdPacket( from );
        return;
...
void CL_InitRenderer( void )
{
    ...
    // true type fonts
    re.RegisterFont( "gfx/fonts/FreeMonoBold.ttf", 24, &cls.consoleFont );
...
void CL_Init( void )
{
    ...
    cl_consoleKeys = Cvar_Get( "cl_consoleKeys", "~ `", CVAR_ARCHIVE );
