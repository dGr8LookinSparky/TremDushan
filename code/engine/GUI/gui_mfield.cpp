...
void MenuField_Init( menufield_s* m )
{
    ...
    if( m->_generic.flags & QMF_SMALLFONT )
    {
        w = SMALLCHAR_WIDTH;
        h = SMALLCHAR_HEIGHT;
    }
    ...
    m->_generic.left = m->_generic.x - l;
    m->_generic.top = m->_generic.y;
    m->_generic.right = m->_generic.x + w + m->field.widthInChars * w;
    m->_generic.bottom = m->_generic.y + h;
}
...
void MenuField_Draw( menufield_s* f )
{
    ...
    if( f->_generic.flags & QMF_SMALLFONT )
    {
        w = SMALLCHAR_WIDTH;
        h = SMALLCHAR_HEIGHT;
        style = UI_SMALLFONT;
    ...
    if( Menu_ItemAtCursor( f->_generic.parent ) == f )
    {
        focus = true;
        style |= UI_PULSE;
    }
    ...
    if( f->_generic.flags & QMF_GRAYED )
    ...
    {
        //draw hightlight
        UI_FillRect( f->_generic.left, f->_generic.top, f->_generic.right - f->_generic.left + 1,
                     f->_generic.bottom - f->_generic.top + 1, listbar_color );
        UI_FillRect( f->_generic.left, f->_generic.bottom, f->_generic.right - f->_generic.left + 1, 1, listbar_color );
        UI_FillRect( f->_generic.left, f->_generic.top, f->_generic.right - f->_generic.left + 1, 1, listbar_color );
        
        
        // draw cursor
        //  UI_FillRect(f->_generic.left, f->_generic.top, f->_generic.right - f->_generic.left + 1,                    f->_generic.bottom - f->_generic.top + 1, listbar_color);
        ...
        //  width = ( UI_Text_Width("_", 0.25f, 0, &uis.freeSansFont)) + 1;
        //  offset = width + f->field.cursor * width;
        //  UI_Text_Paint(x + offset, y+9, 0.25f, color,  "_", 0, 0,  UI_BLINK | UI_RIGHT,  &uis.freeSansFont);
    ...
    if( f->_generic.name )
    {
        //UI_DrawString(x - w, y, f->_generic.name, style | UI_RIGHT, color);
        UI_Text_Paint( x - w, y + 8, 0.25f, color, f->_generic.name, 0, 0, style | UI_RIGHT | UI_DROPSHADOW, &uis.freeSansFont );
    ...
    switch( keycode )
    {
        ...
        default:
            if( keycode & K_CHAR_FLAG )
            {
                ...
                if( ( m->_generic.flags & QMF_UPPERCASE ) && Q_islower( keycode ) )
                    keycode -= 'a' - 'A';
                else if( ( m->_generic.flags & QMF_LOWERCASE ) && Q_isupper( keycode ) )
                    keycode -= 'A' - 'a';
                else if( ( m->_generic.flags & QMF_NUMBERSONLY ) && Q_isalpha( keycode ) )
