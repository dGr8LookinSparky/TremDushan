...
=============
*/
bool ACEAI_InFront( gentity_t* self, gentity_t* other )
{
    vec3_t          vec;
    float           angle;
    vec3_t          forward;
...
=============
*/
bool ACEAI_Visible( gentity_t* self, gentity_t* other )
{
    vec3_t          spot1;
    vec3_t          spot2;
    trace_t         trace;
...
bool ACEAI_FindEnemy( gentity_t* self )
{
    ...
    for( i = 0; i < g_maxclients.integer; i++ )
    {
        ...
        if( player->client->noclip || player->client->ps.pm_type == PM_NOCLIP )
            continue;
...
bool ACEAI_CheckShot( gentity_t* self )
{
    trace_t         tr;
