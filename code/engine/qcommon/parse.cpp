...
static script_t* Parse_LoadScriptFile( const char* filename )
{
    ...
    length = FS_FOpenFileRead( pathname, &fp, false );
    if( !fp )
        return NULL;
...
static int Parse_Directive_elif( source_t* source )
{
    ...
    if( !Parse_Evaluate( source, &value, NULL, true ) )
        return false;
    skip = ( value == 0 );
...
static int Parse_Directive_evalfloat( source_t* source )
{
    ...
    if( !Parse_Evaluate( source, NULL, &value, false ) )
        return false;
    token.line = source->scriptstack->line;
    token.whitespace_p = source->scriptstack->script_p;
    token.endwhitespace_p = source->scriptstack->script_p;
...
static int Parse_DollarDirective_evalint( source_t* source )
{
    ...
    if( !Parse_DollarEvaluate( source, &value, NULL, true ) )
        return false;
    //
    token.line = source->scriptstack->line;
    token.whitespace_p = source->scriptstack->script_p;
...
static int Parse_DollarDirective_evalfloat( source_t* source )
{
    ...
    if( !Parse_DollarEvaluate( source, NULL, &value, false ) )
        return false;
    token.line = source->scriptstack->line;
    token.whitespace_p = source->scriptstack->script_p;
    token.endwhitespace_p = source->scriptstack->script_p;
