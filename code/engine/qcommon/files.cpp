...
"home path" points to ~/.OpenWolf or similar
...
When building a pak file, make sure a OpenWolf.cfg isn't present in it,
...
typedef struct
{
    char            pakFilename[MAX_OSPATH];	// c:\OpenWolf\base\pak0.pk3
...
typedef struct
{
    char            path[MAX_OSPATH];	// c:\OpenWolf
...
int FS_FOpenFileRead( const char* filename, fileHandle_t* file, bool uniqueFILE )
{
    ...
    // make sure the OpenWolfkey file is only readable by the OpenWolf.exe at initialization
    // any other time the key should only be accessed in memory using the provided functions
    if( com_fullyInitialized && strstr( filename, "OpenWolfkey" ) )
    {
        *file = 0;
        return -1;
    }
...
bool FS_ComparePaks( char* neededpaks, int len, bool dlstring )
{
    ...
    *neededpaks = 0;
    ...
    for( i = 0; i < fs_numServerReferencedPaks; i++ )
    {
        ...
        // Make sure the server cannot make us write to non-OpenWolf directories.
...
void FS_Restart( int checksumFeed )
{
    ...
    {
        // skip the OpenWolf.cfg if "safe" is on the command line
        if( !Com_SafeMode() )
        {
...
bool FS_ConditionalRestart( int checksumFeed )
{
    ...
    {
        Com_GameRestart( checksumFeed, false );
        return true;
    }
...
void FS_FilenameCompletion( const char* dir, const char* ext, bool stripExt, void ( *callback )( const char* s ) )
{
    char**          filenames;
    int             nfiles;
    int             i;
