...
Ported to OpenWolf by Andrew "DerSaidin" Browne.
...
bool LoadLuaFile( char* path, int num_vm, vmType_t type )
{
    int             flen = 0;
    char*           code;
    //char           *signature;
...
bool G_LuaInit()
{
    ...
    numFiles = 0;
    numdirs = trap_FS_GetFileList( "scripts/", ".lua", dirlist, 1024 );
    dirptr = dirlist;
    ...
    sprintf( filename, "scripts/%s/", buff );
 ...
 */
bool G_LuaCall( lua_vm_t* vm, char* func, int nargs, int nresults )
{
 ...
 * If the function does not exist, returns false.
 */
bool G_LuaGetNamedFunction( lua_vm_t* vm, char* name )
{
    if( vm->L )
    {
...
bool G_LuaStartVM( lua_vm_t* vm )
{
    ...
    if( lua_istable( vm->L, -1 ) )
    {
        lua_pushstring( vm->L, va( "%s%s%sbase/scripts/%s?.lua;%s%s%s%slualib%slua%s?.lua",
                                   homepath, LUA_DIRSEP, gamepath, LUA_DIRSEP,
                                   homepath, LUA_DIRSEP, gamepath, LUA_DIRSEP, LUA_DIRSEP, LUA_DIRSEP ) );
        lua_setfield( vm->L, -2, "path" );
        lua_pushstring( vm->L, va( "%s%s%s%s?.%s;%s%s%s%slualib%sclibs%s?.%s",
...
/** G_LuaStopVM( vm )
 * Stops one virtual machine, and calls its Quit callback.
 */
void G_LuaStopVM( lua_vm_t* vm )
{
    ...
    if( vm->L )
    {
        if( G_LuaGetNamedFunction( vm, "Quit" ) )
            G_LuaCall( vm, "Quit", 0, 0 );
        lua_close( vm->L );
        vm->L = NULL;
    }
    if( vm->id >= 0 )
...
/** G_LuaHook_MapStart
 * MapStart( mapname, restart ) callback
 */
void G_LuaHook_MapStart( char* mapname, int restart )
{
    int             i;
    lua_vm_t*       vm;
    ...
    for( i = 0; i < LUA_NUM_VM; i++ )
    {
        ...
        if( vm )
        {
            if( vm->id < 0 /*|| vm->err */ )
                continue;
            if( !G_LuaGetNamedFunction( vm, "MapStart" ) )
                continue;
            // Arguments
            lua_pushstring( vm->L, mapname );
            lua_pushinteger( vm->L, restart );
            // Call
            if( !G_LuaCall( vm, "MapStart", 2, 0 ) )
            {
                //G_LuaStopVM(vm);
                continue;
            }
...
/** G_LuaHook_InitGame
 * InitGame( levelTime, randomSeed, restart ) callback
 */
void G_LuaHook_InitGame( int levelTime, int randomSeed, int restart )
{
    ...
    for( i = 0; i < LUA_NUM_VM; i++ )
    {
        ...
        if( vm )
        {
            if( vm->id < 0 /*|| vm->err */ )
                continue;
            if( !G_LuaGetNamedFunction( vm, "InitGame" ) )
                continue;
            // Arguments
...
void G_LuaHook_ShutdownGame( int restart )
{
    ...
    for( i = 0; i < LUA_NUM_VM; i++ )
    {
        ...
        if( vm )
        {
            if( vm->id < 0 /*|| vm->err */ )
                continue;
            if( !G_LuaGetNamedFunction( vm, "ShutdownGame" ) )
                continue;
            // Arguments
            lua_pushinteger( vm->L, restart );
            // Call
...
void G_LuaHook_RunFrame( int levelTime )
{
    ...
    for( i = 0; i < LUA_NUM_VM; i++ )
    {
        ...
        if( vm )
        {
            if( vm->id < 0 /*|| vm->err */ )
                continue;
            if( !G_LuaGetNamedFunction( vm, "RunFrame" ) )
                continue;
            // Arguments
            lua_pushinteger( vm->L, levelTime );
            // Call
 ...
 * rejectreason = ClientConnect( clientNum, firstTime, isBot ) callback
 */
bool G_LuaHook_ClientConnect( int clientNum, bool firstTime, bool isBot, char* reason )
{
    int             i;
    lua_vm_t*       vm;
    ...
    for( i = 0; i < LUA_NUM_VM; i++ )
    {
        ...
        if( vm )
        {
            if( vm->id < 0 /*|| vm->err */ )
                continue;
            if( !G_LuaGetNamedFunction( vm, "ClientConnect" ) )
                continue;
            // Arguments
            ...
            // Call
            if( !G_LuaCall( vm, "ClientConnect", 3, 1 ) )
            {
                //G_LuaStopVM(vm);
                continue;
            }
...
void G_LuaHook_ClientDisconnect( int clientNum )
{
    ...
    for( i = 0; i < LUA_NUM_VM; i++ )
    {
        ...
        if( vm )
        {
            if( vm->id < 0 /*|| vm->err */ )
                continue;
            if( !G_LuaGetNamedFunction( vm, "ClientDisconnect" ) )
                continue;
            // Arguments
            lua_pushinteger( vm->L, clientNum );
            // Call
            if( !G_LuaCall( vm, "ClientDisconnect", 1, 0 ) )
            {
                //G_LuaStopVM(vm);
                continue;
            }
...
void G_LuaHook_ClientBegin( int clientNum )
{
    ...
    for( i = 0; i < LUA_NUM_VM; i++ )
    {
        ...
        if( vm )
        {
            if( vm->id < 0 /*|| vm->err */ )
                continue;
            if( !G_LuaGetNamedFunction( vm, "ClientBegin" ) )
                continue;
            // Arguments
            lua_pushinteger( vm->L, clientNum );
            // Call
            if( !G_LuaCall( vm, "ClientBegin", 1, 0 ) )
            {
                //G_LuaStopVM(vm);
                continue;
            }
...
void G_LuaHook_ClientUserinfoChanged( int clientNum )
{
    ...
    for( i = 0; i < LUA_NUM_VM; i++ )
    {
        ...
        if( vm )
        {
            if( vm->id < 0 /*|| vm->err */ )
                continue;
            if( !G_LuaGetNamedFunction( vm, "ClientUserinfoChanged" ) )
                continue;
            // Arguments
            lua_pushinteger( vm->L, clientNum );
            // Call
            if( !G_LuaCall( vm, "ClientUserinfoChanged", 1, 0 ) )
            {
                //G_LuaStopVM(vm);
                continue;
            }
 ...
 */
//OpenWolf most of the params gone.
void G_LuaHook_ClientSpawn( int clientNum )
{
    ...
    for( i = 0; i < LUA_NUM_VM; i++ )
    {
        ...
        if( vm )
        {
            if( vm->id < 0 /*|| vm->err */ )
                continue;
            if( !G_LuaGetNamedFunction( vm, "ClientSpawn" ) )
                continue;
            // Arguments
            lua_pushinteger( vm->L, clientNum );
            /*
            ...
            */
            // Call
            if( !G_LuaCall( vm, "ClientSpawn", 1, 0 ) )
            {
                //G_LuaStopVM(vm);
                continue;
            }
 ...
 */
bool G_LuaHook_ClientCommand( int clientNum, char* command )
{
    int             i;
    lua_vm_t*       vm;
    ...
    for( i = 0; i < LUA_NUM_VM; i++ )
    {
        ...
        if( vm )
        {
            if( vm->id < 0 /*|| vm->err */ )
                continue;
            if( !G_LuaGetNamedFunction( vm, "ClientCommand" ) )
                continue;
            // Arguments
            lua_pushinteger( vm->L, clientNum );
            lua_pushstring( vm->L, command );
            // Call
            if( !G_LuaCall( vm, "ClientCommand", 2, 1 ) )
            {
                //G_LuaStopVM(vm);
                continue;
            }
 ...
 * intercepted = ConsoleCommand( command ) callback
 */
bool G_LuaHook_ConsoleCommand( char* command )
{
    int             i;
    lua_vm_t*       vm;
    ...
    for( i = 0; i < LUA_NUM_VM; i++ )
    {
        ...
        if( vm )
        {
            if( vm->id < 0 /*|| vm->err */ )
                continue;
            if( !G_LuaGetNamedFunction( vm, "ConsoleCommand" ) )
                continue;
            // Arguments
            lua_pushstring( vm->L, command );
            // Call
            if( !G_LuaCall( vm, "ConsoleCommand", 1, 1 ) )
            {
                //G_LuaStopVM(vm);
                continue;
            }
...
for(i = 0; i < LUA_NUM_VM; i++)
{
	...
	if(vm)
	{
		if(vm->id < 0 )
			continue;
		if(!G_LuaGetNamedFunction(vm, "Print"))
			continue;
		// Arguments
		lua_pushstring(vm->L, text);
		// Call
		if(!G_LuaCall(vm, "Print", 1, 0))
		{
			//G_LuaStopVM(vm);
			continue;
		}
...
/** G_LuaHook_Obituary
 * (customObit) = Obituary( victim, killer, meansOfDeath ) callback
 */
bool G_LuaHook_Obituary( int victim, int killer, int meansOfDeath, char* customObit )
{
    int             i;
    lua_vm_t*       vm;
    ...
    for( i = 0; i < LUA_NUM_VM; i++ )
    {
        ...
        if( vm )
        {
            if( vm->id < 0 /*|| vm->err */ )
                continue;
            if( !G_LuaGetNamedFunction( vm, "Obituary" ) )
                continue;
            // Arguments
 ...
 * <function>( entitynum );
 */
bool G_LuaHook_EntityThink( char* function, int entity )
{
    int             i;
    lua_vm_t*       vm;
 ...
 */
bool G_LuaHook_EntityTouch( char* function, int entity, int other )
{
    int             i;
    lua_vm_t*       vm;
 ...
 */
bool G_LuaHook_EntityUse( char* function, int entity, int other, int activator )
{
    int             i;
    lua_vm_t*       vm;
 ...
 */
bool G_LuaHook_EntityHurt( char* function, int entity, int inflictor, int attacker )
{
    int             i;
    lua_vm_t*       vm;
 ...
 */
bool G_LuaHook_EntityDie( char* function, int entity, int inflictor, int attacker, int dmg, int mod )
{
    int             i;
    lua_vm_t*       vm;
 ...
 * <function>( entitynum );
 */
bool G_LuaHook_EntityFree( char* function, int entity )
{
    int             i;
    lua_vm_t*       vm;
 ...
 * <function>( entitynum, other );
 */
bool G_LuaHook_EntityTrigger( char* function, int entity, int other )
{
    int             i;
    lua_vm_t*       vm;
 ...
 * <function>( entitynum );
 */
bool G_LuaHook_EntitySpawn( char* function, int entity )
{
    int             i;
    lua_vm_t*       vm;
