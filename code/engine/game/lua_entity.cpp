...
static int entity_SetOrigin( lua_State* L )
{
    ...
    //G_SetOrigin(lent->e, lua_getvector(L, 2));
    VectorCopy( lua_getvector( L, 2 ), lent->e->s.origin );
...
static int entity_GetClientName( lua_State* L )
{
    ...
    lent = lua_getentity( L, 1 );
    if( !lent->e->client )
        return luaL_error( L, "`GetClientName' must be used with a client entity" );
...
static const luaL_reg entity_meta[] =
{
    ...
    // TODO: move client entity stuff to lua_client?
